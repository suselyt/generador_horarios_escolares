...
class GeneradorHorarios {
    ...
    // ================== FUNCIÓN CORREGIDA: Generar horarios grupales ==================
    generarHorariosGrupales() {
        console.log("\n=== GENERANDO HORARIOS GRUPALES ===");

        // Primero, limpiar todos los horarios grupales
        for (const grupo of this.grupos) {
            let bloqueInicio, bloqueFin;
            if (grupo.turno === "Matutino") {
                bloqueInicio = 1;
            ...
            for (const dia of this.dias) {
                for (let bloque = bloqueInicio; bloque <= bloqueFin; bloque++) {
                    grupo.horario[dia][bloque] = { materia: null, abreviatura: null, docente: null, aula: null };
                }
            }
        }

        // Ahora recorrer todos los horarios de profesores y asignar correctamente
        for (const profesor of this.profesores) {
            for (const dia of this.dias) {
                for (let bloque = 1; bloque <= this.totalBloques; bloque++) {
                    const asignacionProfesor = profesor.horario[dia][bloque];

                    // Si el profesor no tiene nada asignado en este bloque, continuar
                    if (!asignacionProfesor.materia) continue;

                    // CASO 1: Extracurriculares - Solo para grupos matutinos
                    if (asignacionProfesor.materia.toLowerCase().includes("extracurricular")) {
                        // Extraer el semestre del texto de la materia
                        const matchSemestre = asignacionProfesor.materia.match(/(\d+)° Semestre/);
                        if (matchSemestre) {
                            const semestre = parseInt(matchSemestre[1]);
                            
                            // Encontrar todos los grupos matutinos de ese semestre
                            const gruposMatutinosSemestre = this.grupos.filter(g => 
                                g.semestre === semestre && 
                                g.turno === "Matutino" && 
                                this.validarTurnoGrupo(g, bloque)
                            );

                            // Asignar extracurricular a todos esos grupos
                            for (const grupo of gruposMatutinosSemestre) {
                                grupo.horario[dia][bloque] = {
                                    materia: "Extracurricular",
                                    abreviatura: asignacionProfesor.abreviatura || "EXT",
                                    docente: profesor.nombre,
                                    aula: null
                                };
                            }
                            
                            console.log(`✓ Extracurricular asignada - ${dia} Bloque ${bloque} - Semestre ${semestre} (${gruposMatutinosSemestre.length} grupos)`);
                        }
                    }
                    // CASO 2: Materias normales (módulos profesionales, tronco común, tutorías)
                        ...
                        if (grupo && this.validarTurnoGrupo(grupo, bloque)) {
                            // Verificar que el bloque esté dentro del rango válido del grupo
                            let bloqueInicio, bloqueFin;
                            if (grupo.turno === "Matutino") {
                                bloqueInicio = 1;
                                bloqueFin = this.config.bloque_fin_matutino;
                            ...
                            if (bloque >= bloqueInicio && bloque <= bloqueFin) {
                                grupo.horario[dia][bloque] = {
                                    materia: asignacionProfesor.materia,
                                    abreviatura: asignacionProfesor.abreviatura,
                                    docente: profesor.nombre,
                                    aula: null
                                };
                                
                                console.log(`✓ ${asignacionProfesor.materia} asignada - ${grupo.nomenclatura} - ${dia} Bloque ${bloque} - Profesor: ${profesor.nombre}`);
                            }
                        }
                    }
                    // CASO 3: Actividades sin grupo específico (fortalecimiento académico)
                    else {
                        console.log(`ℹ Actividad sin grupo: ${asignacionProfesor.materia} - ${profesor.nombre} - ${dia} Bloque ${bloque}`);
                    }
                }
            }
        }
    ...
    // ================== Función de validación de coherencia ==================
                    ...
                    // Validar materias normales
                        ...
                        if (grupo && this.validarTurnoGrupo(grupo, bloque)) {
                            ...
                            if (!asignacionGrupo?.materia || 
                                ...
                                console.error(`❌ ERROR: Inconsistencia en ${dia} bloque ${bloque} - Profesor: ${asignacionProfesor.materia} vs Grupo ${grupo.nomenclatura}: ${asignacionGrupo?.materia || 'vacío'}`);
                                errores++;
                            }
                        }
                    }
...
try {
        ...
        console.error("Error durante la generación de horarios:", error);
        process.exit(1);
    }.horario[dia][bloque].grupo === grupo.nomenclatura) return true;
        }
        return false;
    }
